...
The resulting model had performance largely level off with a regression coefficient of 0.75. The resulting model had an RMSE of 0.1266806 and an R2 of 0.4346677 which is practically equivalent to the Ridge model
...
From the model summary table above, the best performing models were the XGBoost model and the Cross-Validated Random Forest model.  Both models had an R2 of 0.72, and the XGBoost model had an RMSE of 0.89 while the RF model had an RMSE of 0.094.  

Overall, the performance metrics for these two models were very similar, especially when looking at the test set evaluation.  As such, model selection took into account other selection criteria.  The Random Forest model has better interpretability, but XGBoost excels in other areas such as in its generalization - specifically, XGBoost has built in regularization techniques that can prevent overfitting to data.  When looking at the training R2 and RMSE, the RF model had a higher R2 at 0.68 and a lower RMSE at 0.098 compared to an R2 of 0.64 and an RMSE of 0.10 for the XGBoost model.  As such, it does not appear overfitting was an issue for either model given the higher R2 for the out-of-sample data.  

When looking at variable importance, XGBoost model and Random Forest model show similar variables of importance, with 7 of the top 10 variables overlapping.  However, although top variables of importance are very similar, the XGBoost model has a wider distribution of importance. For example, the XGBoost model and the RF model both have Mnf Flow as the top variable of importance.  We can look at this top variable as a way to normalize the variable importance, with the top score as a reference point.  The XGBoost model has 6 variables with an importance score greater than 50% of the maximum score.  In comparison, when looking at the top 10 variables in the Random Forest model, we can see that there is a steep drop off of importance after Mnf Flow. Besides Mnf Flow, the Random Forest model does not have any variables with greater than 50% of the maximum score.  Given that it is not uncommon for some datapoints to be missing in the datasets, having a wider range of important variables could be favorable in case the imputed values are not completely accurate to the true values of the missing datapoints.  

With a greater distribution of variable importance and similar model performance, the XGBoost model was ultimately selected of choice for pH prediction.

### 7. Conclusion

This project project explored multiple predictive models to address the new regulatory requirement of understanding and predicting PH in the manufacturing process at ABC Beverage. The analysis began with exploratory data analysis to understand variable distributions and relationships, detect outliers, and address missing data. Bagging imputation technique was applied to fill in missing values. Additionally, rows with missing PH values were dropped, and we applied one-hot encoding to the Brand.Code variable to retain its categorical information without introducing imputation risks.
Several models were tested, covering a range of categories: linear models (PLS, Ridge Regression, LASSO, Elastic Net), non-linear models (SVM, Neural Networks), tree-based models (CART, Random Forest, XGBoost), and rule-based hybrid models (Cubist). Each model’s performance was evaluated based on metrics such as RMSE, R², and MAE, focusing on balancing predictive accuracy and interpretability. Among the models, Cubist, Random Forest, and XGBoost demonstrated a strong ability to capture variability. However, the Cubist model, with R² of 0.69, was not selected to predict PH in the manufacturing process due to its limited interpretability and reliance on complex rules. The Random Forest model, with an R² of 0.68, was also not selected as the best model to predict PH  due to the steep drop off of importance after Mnf Flow in the variable importance results. The dominance of this feature might reduce the influence of different variables in the model.
We selected the XGBoost model since it shows good generalization, as the test set metrics outperform the training set. The test RMSE (0.0894) is lower than the training RMSE (0.104), and the test R², 0.719) is higher than the training R², (0.643), indicating better variance explanation on unseen data. Additionally, the lower test MAE (0.0664 vs. 0.0755) further confirms its predictive accuracy.  Manufacturing Flow Rate (Mnf.Flow) was the most influential predictor identified by the XGBoost model, followed by Continuous Usage (Usage.cont). Other key predictors include Oxygen in Filler (Oxygen.Filler), Speed of Filler (Filler.Speed), Liquid Temperature (Temperature), Brand Code (Brand.CodeC), Pressure Vacuum, and Carbonation Pressure (Carb.Pressure1), all of which play critical roles in regulating the pH during production.
